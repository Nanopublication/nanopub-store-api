...
public class NanopubDaoImpl implements NanopubDao
{
	...
	/**
	 * @param nanopub
	 *          The nanopublication to store.
	 * @return the URI that can be used to retrieve the nanopublication.
	 * @throws NanopubDaoException
	 *           if the nanopublication already exists in the store, or when there is an error storing
	 *           the nanopublication in the underlying store.
	 */
	@Override
	public String storeNanopub(Nanopub nanopub) throws NanopubDaoException
	{
		...
		finally
		{
			if (connection != null)
			{
				...
				{
					throw new NanopubDaoException(
						"Error closing repository connection (after storing nanopublication)!",
						e);
				}
			}
		}
	...
	public boolean hasNanopub(String key) throws NanopubDaoException
	{
		...
		try
		{
			connection = this.repository.getConnection();
			result =
				connection.hasStatement(null, DC.IDENTIFIER, object, false,
					STORE_MAPPING_CONTEXT);

		}
		catch (RepositoryException e)
		{
			throw new NanopubDaoException("Error looking for nanopublication existence!",
				e);
		}
		finally
		{
			if (connection != null)
			{
				...
				{
					throw new NanopubDaoException(
						"Error closing connection (after finding np existence)!", e);
				}
			}
		}
	...
	public List<URI> listNanopubs() throws NanopubDaoException
	{
		...
		{
			connection = this.repository.getConnection();
			RepositoryResult<Statement> resultSet =
				connection.getStatements(null, DC.IDENTIFIER, null, false,
					STORE_MAPPING_CONTEXT);
		...
		finally
		{
			if (connection != null)
			{
				...
				{
					throw new NanopubDaoException(
						"Error closing connection (after listing nanopublications)!", e);
				}
			}
		}
	...
	public void storeMappingStatement(URI nanopubUri, String key)
		throws NanopubDaoException
	{
		...
		try
		{
			...
			Statement statement = new StatementImpl(nanopubUri, DC.IDENTIFIER, object);
		...
		finally
		{
			if (connection != null)
			{
				...
				{
					throw new NanopubDaoException(
						"Error closing connection (storing uri to key mapping statement)!", e);
				}
			}
		}
	}
	...
	private URI retrieveNanopubURI(String key) throws NanopubDaoException
	{
		...
		try
		{
			...
			RepositoryResult<Statement> result =
				connection.getStatements(null, DC.IDENTIFIER, object, true,
					STORE_MAPPING_CONTEXT);
